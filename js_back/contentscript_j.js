...
TranslateObj.prototype = {
    ...
    doNebCallFun: function(func_name, arg_list) {
        var self = this;
        var ret_data;
        if(func_name == "query_foodinfo_by_key") {
            ret_data = self.kitObj.query_foodinfo_by_key(arg_list[0]);
        } else if(func_name == "query_food_list_by_page") {
            ret_data = self.kitObj.query_food_list_by_page(arg_list[0],arg_list[1]);
        } else if(func_name=="query_food_sum"){
            ret_data = self.kitObj.query_food_sum(arg_list[0]);
        ...
        }else if(func_name=="query_hot_food_list"){
            ret_data = self.kitObj.query_hot_food_list(arg_list[0]);
        }
        // 发送结果出去
        window.postMessage({
            "data":{
    ...
    doSendTransaction: function(func_name, arg_list) {
        var self = this;
		var ret_data;
        if (func_name == "add_food_info_to_list") {
            // 调用add_kit_to_list方法
            ret_data=self.kitObj.add_food_info_to_list(arg_list[0]);
